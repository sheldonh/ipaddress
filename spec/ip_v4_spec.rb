...
describe "IP::V4" do
  describe "#new" do
    ...
    it "takes an address/mask dotted quad string" do
      ...
      ip.mask.should == IP::V4.new("192.168.0.0/24").mask
    end

    it "takes an unmasked address string and assumes a mask size of 32" do
      ip = IP::V4.new("192.168.0.1")
      ip.mask.should == 32
    end

    it "takes an integer address and mask size" do
      ip = IP::V4.new(3232235520, 24)
      ip.address.should == "192.168.0.0"
      ip.mask.should == 24
    end

    it "raises an ArgumentError if given no arguments" do
      expect { IP::V4.new }.to raise_error(ArgumentError)
    end

    it "raises an ArgumentError if given more than 2 arguments" do
      expect { IP::V4.new(3232235520, 24, :wombat) }.to raise_error(ArgumentError)
    end
  end

  describe "#<=>" do
    it "returns -1 if the other IP::V4 has an higher integer address" do
      this = IP::V4.new("192.168.0.1")
      other = IP::V4.new("192.168.0.2")
      this.<=>(other).should == -1
    end

    it "returns 1 if the other IP::V4 has a lower integer address" do
      this = IP::V4.new("192.168.0.2")
      other = IP::V4.new("192.168.0.1")
      this.<=>(other).should == 1
    end

    it "returns 0 if the other IP::V4 has the same integer address" do
      this = IP::V4.new("192.168.0.1")
      other = IP::V4.new("192.168.0.1")
      this.<=>(other).should == 0
    end
  ...
  describe "#==" do
    it "is true if the other IP::V4 has the same address and mask size" do
      this = IP::V4.new("192.168.0.0/24")
      other = IP::V4.new("192.168.0.0/24")
      this.should == other
    end

    it "is false if the other IP::V4 has a different address" do
      this = IP::V4.new("192.168.0.1/24")
      other = IP::V4.new("192.168.0.2/24")
      this.should_not == other
    end

    it "is false if the other IP::V4 has a different mask size" do
      this = IP::V4.new("192.168.0.1/24")
      other = IP::V4.new("192.168.0.2/28")
      this.should_not == other
    end
  end
  ...
  describe "#adjacent?" do
    it "is true if the other IP::V4's network immediately follows this one" do
      this = IP::V4.new("192.168.0.0/28")
      other = IP::V4.new("192.168.0.16/28")
      this.adjacent?(other).should be_true
    end

    it "is true if the other IP::V4's network immediately precedes this one" do
      this = IP::V4.new("192.168.0.0/28")
      other = IP::V4.new("192.167.255.254/28")
      this.adjacent?(other).should be_true
    end

    it "is false if the other IP::V4's network includes this one" do
      this = IP::V4.new("192.168.0.16/28")
      other = IP::V4.new("192.168.0.0/24")
      this.adjacent?(other).should be_false
    end

    it "is false if the other IP::V4's network is included in this one" do
      this = IP::V4.new("192.168.0.16/28")
      other = IP::V4.new("192.168.0.0/24")
      this.adjacent?(other).should be_false
    end
  ...
  describe "#broadcast" do
    it "returns the broadcast address as an IP::V4 if :instance is given" do
      ip = IP::V4.new("192.168.0.0/24")
      ip.broadcast(:instance).should == IP::V4.new("192.168.0.255/24")
    end
    ...
    it "defaults to :instance if no presentation is given" do
      ip = IP::V4.new("192.168.0.0/24")
      ip.broadcast.should == IP::V4.new("192.168.0.255/24")
    end

    it "returns itself if it is a broadcast address" do
      ip = IP::V4.new("192.168.0.255/24")
  ...
  describe "#each" do
    it "iterates over each address in the network, including network and broadcast address, if :address is given" do
      ip = IP::V4.new("192.168.0.0/30")
      expected_addresses = [0, 1, 2, 3].collect { |i| IP::V4.new("192.168.0.#{i}/30") }
      addresses = []
    ...
    it "iterates over each host address in the network, excluding network and broadcast address, if :host is given" do
      ip = IP::V4.new("192.168.0.0/30")
      expected_addresses = [1, 2].collect { |i| IP::V4.new("192.168.0.#{i}/30") }
      addresses = []
    ...
    it "iterates over both addresses in a PPP (/31) network if :host is given" do
      ip = IP::V4.new("192.168.0.0/31")
      addresses = []
      ip.each { |i| addresses << i }
      addresses.should == [ IP::V4.new("192.168.0.0/31"), IP::V4.new("192.168.0.1/31") ]
    end
  ...
  describe "#follow?" do
    it "is true if the other IP::V4's network immediately precedes this one" do
      IP::V4.new("192.168.1.0/24").follow?(IP::V4.new("192.168.0.0/24")).should be_true
    end

    it "is false if the other IP::V4's network does not immediately precede this one" do
      IP::V4.new("192.168.1.2/31").follow?(IP::V4.new("192.168.0.0/24")).should be_false
    end
  end
  ...
  describe "#host?" do
    ...
    it "is true if the address is the same as the masked address but the mask size is 32" do
      ip = IP::V4.new("192.168.0.0/32")
      ip.host?.should be_true
    end

    it "is true if the address is the same as the masked address but the mask size is 0" do
      ip = IP::V4.new("192.168.0.0/0")
      ip.host?.should be_true
    end
  end

  describe "#include?" do
    it "is true if the other IP::V4 is a host in this network" do
      network = IP::V4.new("192.168.0.16/28")
      ip = IP::V4.new("192.168.0.17/32")
      network.include?(ip).should be_true
    end

    it "is false if the other IP::V4 is a host outside this network" do
      network = IP::V4.new("192.168.0.16/28")
      ip = IP::V4.new("192.168.0.1/32")
      network.include?(ip).should be_false
    end

    it "is true if the other IP::V4 is a network that fits inside this network" do
      network = IP::V4.new("192.168.0.16/28")
      ip = IP::V4.new("192.168.0.24/29")
      network.include?(ip).should be_true
    end

    it "is false if the other IP::V4 is a network that does not fit inside this network" do
      network = IP::V4.new("192.168.0.16/28")
      ip = IP::V4.new("192.168.0.0/28")
      network.include?(ip).should be_false
    end

    it "is true if the other IP::V4 is a host in this network but its network does not fit inside this network" do
      network = IP::V4.new("192.168.0.16/28")
      ip = IP::V4.new("192.168.0.17/24")
      network.include?(ip).should be_true
    end

    it "is true if this is the unspecified (wildcard) address" do
      wildcard = IP::V4.new("0.0.0.0/0")
      ip = IP::V4.new("192.168.0.1")
  ...
  describe "#network" do
    it "returns the network address as an IP::V4 if :instance is given" do
      ip = IP::V4.new("192.168.0.1/24")
      ip.network(:instance).should == IP::V4.new("192.168.0.0/24")
    end
  ...
  describe "#precede?" do
    it "is true if the other IP::V4's network immediately follows this one" do
      IP::V4.new("192.168.0.0/24").precede?(IP::V4.new("192.168.1.0/24")).should be_true
    end

    it "is false if the other IP::V4's network does not immediately follow this one" do
      IP::V4.new("192.168.0.0/24").precede?(IP::V4.new("192.168.1.2/31")).should be_false
    end
  end
  ...
  describe ".aggregate" do
    ...
    it "copes with unordered addresses if input order is given as :unsorted" do
      ...
      aggregates = IP::V4.aggregate(a, :unsorted)
      aggregates.should == [IP::V4.new("192.168.0.0/27"), IP::V4.new("192.168.0.64/27")]
    end
    ...
    it "does not modify the passed array or its contents" do
      ...
      a.freeze
      expect { aggregates = IP::V4.aggregate(a) }.to_not raise_error
    end
